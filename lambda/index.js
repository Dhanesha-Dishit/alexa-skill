...
function populateRoundAnswers(
  ...
  const answersCopy =
    translatedQuestion[Object.keys(translatedQuestion)[0]].slice();
...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (sessionAttributes.currentQuestionIndex === GAME_LENGTH - 1) {
    ...
    if (supportsDisplay(handlerInput)) {
      let payload = {
        phrase: makePage(
          aplFirstPageSpeechOutput,
          "",
          `<speak>${aplFirstPageSpeechOutput}</speak>`,
          firstTransformerList
        ),
        nextPhrase: makePage(
          aplSecondPageSpeechOutput,
          "",
          `<speak>${aplSecondPageSpeechOutput}</speak>`,
